...
type GolRequest struct {
	Typ GolStructure `json:"typ"`
	At  image.Point  `json:"at"`
}

type GolStructure int

const (
	Glider GolStructure = iota
	GliderGun
	Tumbler
	Clear
)
...
func (b *Board) Start(out chan<- []image.Point) {
	...
	for {
		out <- b.Points()
		b.Iterate()
		// this will throttle, most definitely.
...
func (b *Board) listen() {
	for {
		...
		case req := <-b.Requests:
			...
			if req.Typ == GliderGun {
				b.gosperGliderGunAt(req.At)
			}
			if req.Typ == Tumbler {
				b.tumblerAt(req.At)
			}
...
func (b *Board) init() {
	//b.cells[image.Pt(0, 0)] = true
	//b.cells[image.Pt(1, 0)] = true
	//b.cells[image.Pt(2, 0)] = true
	//
